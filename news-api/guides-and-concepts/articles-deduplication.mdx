...
- The deduplication feature is available only for the
  [Search](/news-api/api-reference/endpoints/search/search-articles-get) endpoint.
- Deduplication supports
  [all languages available in News API](/news-api/api-reference/overview/enumerated-parameters#language-lang-and-not-lang).

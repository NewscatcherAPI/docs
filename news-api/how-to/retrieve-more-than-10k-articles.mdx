...
Both methods available for synchronous and asynchronous clients. **All existing
News API parameters are available for both methods**, giving you full control over
filtering, sorting, and content selection while bypassing the 10,000 article
limit.
...
<Accordion title="Rate limiting errors">
  If you hit rate limits: - Reduce concurrency (for async methods). - Add longer
  delays between requests. - Break large requests into smaller batches.
</Accordion>

{" "}

<Accordion title="Memory errors">
  If you run out of memory: - Reduce `max_articles` parameter. - Process data in
  smaller batches. - Save results incrementally as shown in the advanced
  example. - Release memory with `del` and `gc.collect()`.
</Accordion>
...
- [Advanced query syntax](/news-api/guides-and-concepts/advanced-querying)
- [API rate limits](/news-api/api-reference/overview/rate-limits)

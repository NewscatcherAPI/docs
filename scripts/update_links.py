...
def update_markdown_links(content: str, mappings: Dict[str, str]) -> Tuple[str, int]:
    ...
    def replace_link(match):
        ...
        query = '?' + url.split('?')[1] if '?' in url and '#' not in url.split('?')[1] else ''
...
def process_file(filepath: Path, mappings: Dict[str, str], dry_run: bool = False) -> Tuple[bool, int]:
    """
...
def main():
    ...
    if dry_run:
        ...
        print("3. Commit changes: git add . && git commit -m 'docs: update internal links'")

        ...
        for i, redirect in enumerate(self.redirects):
            ...
            for field in ["source", "destination", "type", "status_code"]:
                if field not in redirect:
                    self.errors.append(
                        f"Redirect #{i+1}: Missing required field '{field}'"
                    )
                    valid = False
            ...
            if not source.startswith("/"):
                self.errors.append(
                    f"Redirect #{i+1}: Source path must start with '/' - got: {source}"
                )
                valid = False
            ...
            if not destination.startswith("/"):
                self.errors.append(
                    f"Redirect #{i+1}: Destination path must start with '/' - got: {destination}"
                )
                valid = False
            ...
            if redirect_type not in ["permanent", "temporary"]:
                self.errors.append(
                    f"Redirect #{i+1}: Type must be 'permanent' or 'temporary' - got: {redirect_type}"
                )
                valid = False
            ...
            if status_code not in [301, 302, 307, 308]:
                self.errors.append(
                    f"Redirect #{i+1}: Status code must be 301, 302, 307, or 308 - got: {status_code}"
                )
                valid = False
            ...
            if redirect_type == "permanent" and status_code not in [301, 308]:
                self.warnings.append(
                    f"Redirect #{i+1}: Type is 'permanent' but status code is {status_code} (expected 301 or 308)"
                )
            if redirect_type == "temporary" and status_code not in [302, 307]:
                self.warnings.append(
                    f"Redirect #{i+1}: Type is 'temporary' but status code is {status_code} (expected 302 or 307)"
                )
    ...
    def check_duplicate_sources(self) -> bool:
        ...
        for i, redirect in enumerate(self.redirects):
            ...
            if source in sources:
                duplicates.append(
                    f"Source '{source}' appears in redirects #{sources[source]+1} and #{i+1}"
                )
            else:
                sources[source] = i
        ...
        for i, redirect in enumerate(self.redirects):
            ...
            if source == destination:
                self_redirects.append(
                    f"Redirect #{i+1}: Source and destination are identical: {source}"
                )
...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python validate_redirects.py redirect-map.json [--schema redirect-map.schema.json]"
        )
        sys.exit(1)

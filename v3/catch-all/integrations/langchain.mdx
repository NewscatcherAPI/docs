...
<Tabs>
  <Tab title="Sync">
    ...
    max_wait_time=1200, # Timeout after 20 minutes (typical jobs: 10-15 min) )
    ```
  </Tab>
  ...
  <Tab title="Async">
    ...
    client = AsyncCatchAllClient(
        ...
        max_wait_time=1200,    # Timeout after 20 minutes
    )
    ```
  </Tab>
</Tabs>
...
- **`catchall_search_data`:** Find NEW articles (10-15 min operation).
...
1. SEARCH: Use `catchall_search_data` ONLY to get a broad initial dataset (e.g.,
   'Find all US office openings').
...
2. ANALYZE: Use `catchall_analyze_data` for ALL follow-up questions.
   - FILTERING & SORTING: 'Show me only Florida deals', 'Sort by date', 'Find
     top 3'.
...
- If the user asks for a subset of data (like 'only Florida deals'), assume it
  is ALREADY in your search results.
- NEVER use `catchall_search_data` to filter data. Always use
  `catchall_analyze_data`.
- Only use `catchall_search_data` if the user explicitly asks for a 'new search'
...
if **name** == "**main**": run_interactive_agent()
...
See the [Monitors documentation](/v3/catch-all/overview/monitors) for complete
usage guide.
...
<CardGroup cols={2}>
  <Card
    ...
    icon="pencil"
    href="/v3/catch-all/how-to/write-effective-queries"
  >
    Learn to construct queries that return focused results
  </Card>
  <Card title="Python SDK" icon="code" href="/v3/catch-all/libraries/python">

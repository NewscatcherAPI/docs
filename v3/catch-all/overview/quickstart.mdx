...
<Steps>
  ...
  <Step title="Create a job">
    ...
    PAYLOAD = {
        ...
        "schema": "Company [NAME] earned [REVENUE] in [QUARTER]"  # Optional template
    }
  ...
  <Step title="Check job status">
    ...
    while True:
        try:
            ...
            completed_step = next((s for s in steps if s['status'] == 'completed' and s.get('completed')), None)
            failed_step = next((s for s in steps if s['status'] == 'failed' and s.get('completed')), None)
    ...
    async function pollJobStatus() {
      while (true) {
        try {
          ...
          const completedStep = steps.find(s => s.status === 'completed' && s.completed);
          const failedStep = steps.find(s => s.status === 'failed' && s.completed);
  ...
  <Step title="Retrieve results">
    ...
    axios
      ...
      .then((response: AxiosResponse<ResultsResponse>) => {
        ...
        console.log(`Valid records: ${results.valid_records}`);
        console.log(`Candidate records: ${results.candidate_records}`);
        console.log(`Duration: ${results.duration}`);

        if (results.all_records.length > 0) {
          const record = results.all_records[0];
          console.log(`\nFirst record: ${record.record_title}`);
          console.log("\nExtracted fields:");

...
1. **Analyze**: Understands your query and generates search queries, validators,
   and extractors.
...
Response schemas are generated uniquely for each query. Field names and
structure in the `enrichment` object vary between jobs—even with identical
inputs.
...
This flexibility lets CatchAll adapt to any query type without predefined
schemas.

<Tip>
  See [Understanding dynamic
  schemas](/v3/catch-all/overview/understanding-dynamic-schemas) to learn how to
...
const URL = "https://catchall.newscatcherapi.com/catchAll/submit";
...
```bash cURL
curl -X POST \
  'https://catchall.newscatcherapi.com/catchAll/submit' \

...
<Steps>
  ...
  <Step title="Enrich">Validates clusters and extracts structured data</Step>
  <Step title="Complete">Delivers records with source citations</Step>
</Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Asynchronous operation" icon="clock">
    ...
    Use the returned `job_id` to poll the job status from [`steps`](http://localhost:3000/v3/catch-all/endpoints/get-job-status#response-steps-status)

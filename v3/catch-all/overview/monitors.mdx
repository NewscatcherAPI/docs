---
title: Monitors
sidebarTitle: Monitors
description: Schedule recurring CatchAll jobs with webhook notifications.
---

Monitors automate data collection by running CatchAll jobs on a schedule. Create
a monitor from a successful job, define when to run it, and receive webhook
notifications when new results are available.

## How it works

Each scheduled execution:

- Creates a new job with its own `job_id`.
- Uses a rolling date window based on schedule frequency.
- Applies the reference job's validators, extractors, and schema.
- Deduplicates records across all runs and merges the results.

<Note>
  Reference job records are **not** included in monitor results. Only records
  from scheduled executions appear in aggregated results.
</Note>

## Before you start

First, create a successful job following the
[Quickstart guide](/v3/catch-all/overview/quickstart).

## Create a monitor

Once your reference job completes with `status: job_completed`, use `job_id` to
create a monitor:

<CodeGroup>

```bash cURL
curl -X POST "https://catchall.newscatcherapi.com/catchAll/monitors/create" \
  -H "x-api-key: YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "reference_job_id": "295b95d8-6041-4f4b-b132-9f009fc6af70",
    "schedule": "every day at 12 PM UTC",
    "webhook": {
      "url": "https://your-endpoint.com/catchall",
      "method": "POST"
    }
  }'
```

```json JSON
{
  "reference_job_id": "295b95d8-6041-4f4b-b132-9f009fc6af70",
  "schedule": "every day at 12 PM UTC",
  "webhook": {
    "url": "https://your-endpoint.com/catchall",
    "method": "POST"
  }
}
```

```python Python
from newscatcher_catchall import CatchAllApi

client = CatchAllApi(api_key="YOUR_API_KEY")

# Create a monitor
monitor = client.monitors.create_monitor(
    reference_job_id="295b95d8-6041-4f4b-b132-9f009fc6af70",
    schedule="every day at 12 PM UTC",
    webhook={
        "url": "https://your-endpoint.com/catchall",
        "method": "POST"
    }
)

print(f"Monitor created: {monitor.monitor_id}")
```

```typescript TypeScript
import { CatchAllApiClient } from "newscatcher-catchall-sdk";

const client = new CatchAllApiClient({ apiKey: "YOUR_API_KEY" });

// Create a monitor
const monitor = await client.monitors.createMonitor({
  referenceJobId: "295b95d8-6041-4f4b-b132-9f009fc6af70",
  schedule: "every day at 12 PM UTC",
  webhook: {
    url: "https://your-endpoint.com/catchall",
    method: "POST",
  },
});

console.log(`Monitor created: ${monitor.monitorId}`);
```

```java Java
import com.newscatcher.catchall.CatchAllApi;
import com.newscatcher.catchall.resources.monitors.requests.CreateMonitorRequestDto;
import com.newscatcher.catchall.types.WebhookDto;

CatchAllApi client = CatchAllApi.builder()
    .apiKey("YOUR_API_KEY")
    .build();

// Create a monitor
var monitor = client.monitors().createMonitor(
    CreateMonitorRequestDto.builder()
        .referenceJobId("295b95d8-6041-4f4b-b132-9f009fc6af70")
        .schedule("every day at 12 PM UTC")
        .webhook(WebhookDto.builder()
            .url("https://your-endpoint.com/catchall")
            .method("POST")
            .build())
        .build()
);

System.out.println("Monitor created: " + monitor.getMonitorId().orElse("N/A"));
```

</CodeGroup>

Response:

```json
{
  "monitor_id": "3fec5b07-8786-46d7-9486-d43ff67eccd4",
  "status": "Monitor Created Successfully"
}
```

<Note>
  The `webhook` parameter is optional. For webhook payload structure and
  authentication options, see the [Webhooks](#webhooks) section.
</Note>

### Update monitor webhook

Modify webhook configuration for an existing monitor without recreating it:

<CodeGroup>
```bash cURL
curl -X PATCH "https://catchall.newscatcherapi.com/catchAll/monitors/{monitor_id}" \
  -H "x-api-key: YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "webhook": {
      "url": "https://new-endpoint.com/webhook",
      "method": "POST",
      "headers": {"Authorization": "Bearer NEW_TOKEN"}
    }
  }'
```
```python Python
from newscatcher_catchall import CatchAllApi

client = CatchAllApi(api_key="YOUR_API_KEY")
monitor_id = "3fec5b07-8786-46d7-9486-d43ff67eccd4"

# Update webhook configuration
response = client.monitors.update_monitor(
    monitor_id=monitor_id,
    webhook={
        "url": "https://new-endpoint.com/webhook",
        "method": "POST",
        "headers": {"Authorization": "Bearer NEW_TOKEN"}
    }
)
print(response)
```
```typescript TypeScript
import { CatchAllApiClient } from "newscatcher-catchall-sdk";

const client = new CatchAllApiClient({ apiKey: "YOUR_API_KEY" });
const monitorId = "3fec5b07-8786-46d7-9486-d43ff67eccd4";

// Update webhook configuration
const response = await client.monitors.updateMonitor(monitorId, {
  webhook: {
    url: "https://new-endpoint.com/webhook",
    method: "POST",
    headers: { Authorization: "Bearer NEW_TOKEN" }
  }
});
console.log(response);
```
```java Java
import com.newscatcher.catchall.CatchAllApi;

CatchAllApi client = CatchAllApi.builder()
    .apiKey("YOUR_API_KEY")
    .build();

String monitorId = "3fec5b07-8786-46d7-9486-d43ff67eccd4";

// Update webhook configuration
var response = client.monitors().updateMonitor(
    monitorId,
    UpdateMonitorRequestDto.builder()
        .webhook(WebhookDto.builder()
            .url("https://new-endpoint.com/webhook")
            .method("POST")
            .headers(Map.of("Authorization", "Bearer NEW_TOKEN"))
            .build())
        .build()
);
System.out.println(response);
```

</CodeGroup>

Response:
```json
{
  "monitor_id": "3fec5b07-8786-46d7-9486-d43ff67eccd4",
  "status": "Monitor updated Successfully"
}
```

<Note>
  **Schedule and reference job cannot be modified.** To change the query or schedule, 
  create a new monitor.
</Note>

## Retrieve results

### List all monitors

Get all monitors for your API key:

<CodeGroup>

```bash cURL
curl "https://catchall.newscatcherapi.com/catchAll/monitors" \
  -H "x-api-key: YOUR_API_KEY"
```

```python Python
from newscatcher_catchall import CatchAllApi

client = CatchAllApi(api_key="YOUR_API_KEY")

# List all monitors
monitors = client.monitors.list_monitors()

print(f"Total monitors: {monitors.total_monitors}")
for monitor in monitors.monitors:
    print(f"{monitor.monitor_id}: {monitor.reference_job_query}")
    print(f"  Enabled: {monitor.enabled}, Schedule: {monitor.schedule}")
```

```typescript TypeScript
import { CatchAllApiClient } from "newscatcher-catchall-sdk";

const client = new CatchAllApiClient({ apiKey: "YOUR_API_KEY" });

// List all monitors
const monitors = await client.monitors.listMonitors();

console.log(`Total monitors: ${monitors.totalMonitors}`);
monitors.monitors.forEach((monitor) => {
  console.log(`${monitor.monitorId}: ${monitor.referenceJobQuery}`);
  console.log(`  Enabled: ${monitor.enabled}, Schedule: ${monitor.schedule}`);
});
```

```java Java
import com.newscatcher.catchall.CatchAllApi;
import com.newscatcher.catchall.types.ListMonitorsResponseDto;

CatchAllApi client = CatchAllApi.builder()
    .apiKey("YOUR_API_KEY")
    .build();

// List all monitors
ListMonitorsResponseDto monitors = client.monitors().listMonitors();

System.out.println("Total monitors: " + monitors.getTotalMonitors());
monitors.getMonitors().forEach(monitor -> {
    System.out.println(monitor.getMonitorId() + ": " + monitor.getReferenceJobQuery());
    System.out.println("  Enabled: " + monitor.getEnabled() + ", Schedule: " + monitor.getSchedule());
});
```

</CodeGroup>

<Expandable title="Response structure">

```json
{
  "total_monitors": 2,
  "monitors": [
    {
      "monitor_id": "0bcbf554-1f38-460e-9f6d-4bb9338560a4",
      "reference_job_id": "2acada6a-3e55-423d-9406-00f5c6dc73da",
      "reference_job_query": "Cross-border mergers and acquisitions involving US companies",
      "enabled": false,
      "schedule": "every day at 12 PM UTC",
      "timezone": "UTC",
      "created_at": "2025-11-04T21:04:22Z"
    },
    {
      "monitor_id": "3fec5b07-8786-46d7-9486-d43ff67eccd4",
      "reference_job_id": "295b95d8-6041-4f4b-b132-9f009fc6af70",
      "reference_job_query": "AI company acquisitions and mergers",
      "enabled": true,
      "schedule": "every 5 minutes",
      "timezone": "UTC",
      "created_at": "2025-11-07T10:54:42Z"
    }
  ]
}
```

**Key fields:**

- `enabled` (boolean): Whether the monitor is currently active
- `reference_job_query` (string): Original query text from the reference job
- `schedule` (string): Natural language schedule description
- `created_at` (string): Monitor creation timestamp

</Expandable>

### List monitor jobs

Get execution history for a specific monitor:

<CodeGroup>

```bash cURL
curl "https://catchall.newscatcherapi.com/catchAll/monitors/{monitor_id}/jobs" \
  -H "x-api-key: YOUR_API_KEY"
```

```python Python
from newscatcher_catchall import CatchAllApi

client = CatchAllApi(api_key="YOUR_API_KEY")
monitor_id = "3fec5b07-8786-46d7-9486-d43ff67eccd4"

# List monitor jobs
jobs = client.monitors.list_monitor_jobs(monitor_id)

print(f"Total jobs: {jobs.total_jobs}")
for job in jobs.jobs:
    print(f"Job {job.job_id}")
    print(f"  Started: {job.start_date}")
    print(f"  Ended: {job.end_date}")
```

```typescript TypeScript
import { CatchAllApiClient } from "newscatcher-catchall-sdk";

const client = new CatchAllApiClient({ apiKey: "YOUR_API_KEY" });
const monitorId = "3fec5b07-8786-46d7-9486-d43ff67eccd4";

// List monitor jobs
const jobs = await client.monitors.listMonitorJobs(monitorId);

console.log(`Total jobs: ${jobs.totalJobs}`);
jobs.jobs.forEach((job) => {
  console.log(`Job ${job.jobId}`);
  console.log(`  Started: ${job.startDate}`);
  console.log(`  Ended: ${job.endDate}`);
});
```

```java Java
import com.newscatcher.catchall.CatchAllApi;

CatchAllApi client = CatchAllApi.builder()
    .apiKey("YOUR_API_KEY")
    .build();

String monitorId = "3fec5b07-8786-46d7-9486-d43ff67eccd4";

// List monitor jobs
var jobs = client.monitors().listMonitorJobs(monitorId);

System.out.println("Total jobs: " + jobs.getTotalJobs());
jobs.getJobs().forEach(job -> {
    System.out.println("Job " + job.getJobId());
    System.out.println("  Started: " + job.getStartDate());
    System.out.println("  Ended: " + job.getEndDate());
});
```

</CodeGroup>

<Expandable title="Response structure">

```json
{
  "monitor_id": "3fec5b07-8786-86d7-9486-d43ff67eccd4",
  "sort_order": "asc",
  "total_jobs": 2,
  "jobs": [
    {
      "job_id": "8a9763dd-3611-4b3b-a6cf-3e893a0c6746",
      "start_date": "2025-11-07T10:50:00Z",
      "end_date": "2025-11-07T10:55:00Z"
    },
    {
      "job_id": "288387df-7e05-4722-83cc-ecbebb6d8123",
      "start_date": "2025-11-07T10:55:00Z",
      "end_date": "2025-11-07T11:00:00Z"
    }
  ]
}
```

Use the `job_id` to retrieve detailed results for a specific execution via
`/catchAll/pull/{job_id}`.

</Expandable>

### Get aggregated results

Retrieve all deduplicated records from all monitor jobs:

<CodeGroup>

```bash cURL
curl "https://catchall.newscatcherapi.com/catchAll/monitors/pull/{monitor_id}" \
  -H "x-api-key: YOUR_API_KEY"
```

```python Python
from newscatcher_catchall import CatchAllApi

client = CatchAllApi(api_key="YOUR_API_KEY")
monitor_id = "3fec5b07-8786-46d7-9486-d43ff67eccd4"

# Get aggregated results
results = client.monitors.pull_monitor_results(monitor_id)

print(f"Monitor: {results.monitor_id}")
print(f"Schedule: {results.cron_expression} ({results.timezone})")
print(f"Total records: {results.records}")
print(f"Date range: {results.run_info}")

for record in results.all_records:
    print(f"\n{record.record_title}")
    print(f"  Citations: {len(record.citations)}")
```

```typescript TypeScript
import { CatchAllApiClient } from "newscatcher-catchall-sdk";

const client = new CatchAllApiClient({ apiKey: "YOUR_API_KEY" });
const monitorId = "3fec5b07-8786-46d7-9486-d43ff67eccd4";

// Get aggregated results
const results = await client.monitors.pullMonitorResults(monitorId);

console.log(`Monitor: ${results.monitorId}`);
console.log(`Schedule: ${results.cronExpression} (${results.timezone})`);
console.log(`Total records: ${results.records}`);
console.log(`Run info:`, results.runInfo);

results.allRecords.forEach((record) => {
  console.log(`\n${record.recordTitle}`);
  console.log(`  Citations: ${record.citations.length}`);
});
```

```java Java
import com.newscatcher.catchall.CatchAllApi;
import com.newscatcher.catchall.types.PullMonitorResponseDto;

CatchAllApi client = CatchAllApi.builder()
    .apiKey("YOUR_API_KEY")
    .build();

String monitorId = "3fec5b07-8786-46d7-9486-d43ff67eccd4";

// Get aggregated results
PullMonitorResponseDto results = client.monitors().pullMonitorResults(monitorId);

System.out.println("Monitor: " + results.getMonitorId());
System.out.println("Schedule: " + results.getCronExpression() + " (" + results.getTimezone() + ")");
System.out.println("Total records: " + results.getRecords());
System.out.println("Run info: " + results.getRunInfo());

results.getAllRecords().forEach(record -> {
    System.out.println("\n" + record.getRecordTitle());
    System.out.println("  Citations: " + record.getCitations().size());
});
```

</CodeGroup>

<Expandable title="Response structure">

```json
{
  "monitor_id": "3fec5b07-8786-46d7-9486-d43ff67eccd4",
  "cron_expression": "*/5 * * * *",
  "timezone": "UTC",
  "reference_job": {
    "query": "AI company acquisitions and mergers",
    "context": "Focus on technology companies, include deal size if mentioned"
  },
  "run_info": {
    "first_run": "2025-11-07T10:50:00Z",
    "last_run": "2025-11-07T11:30:00Z"
  },
  "records": 2,
  "status": "Done",
  "all_records": [
    {
      "record_id": "6417909601438475967",
      "record_title": "Samsung Electronics Acquires Oxford Semantic Technologies",
      "added_on": "2025-11-07T10:55:00Z",
      "enrichment": {
        "acquiring_company": "Samsung Electronics",
        "acquired_company": "Oxford Semantic Technologies",
        "acquisition_date": "2024",
        "deal_type": "acquisition",
        "ai_technology_focus": ["knowledge graphs", "personalized AI"],
        "confidence": "high"
      },
      "citations": [
        {
          "title": "Samsung invests in AI and M&A",
          "link": "https://n.news.naver.com/mnews/article/629/0000441563",
          "published_date": "2025-11-07T11:15:11Z",
          "job_id": "c4cb35e9-c8a5-46bc-87aa-5fdbf36f8e33",
          "added_on": "2025-11-07T10:55:00Z"
        }
      ],
      "added_on": "2025-11-07T10:55:00Z",
      "updated_on": "2025-11-07T10:55:00Z"
    }
  ]
}
```

**Key fields:**

- `cron_expression` (string): Parsed cron format from your natural language
  schedule
- `reference_job` (object): Original query and context from the reference job
- `run_info` (object): Execution timeframe showing first and last monitor runs
- `all_records` (array): Deduplicated records from all monitor executions

</Expandable>

## Manage monitors

### Enable a monitor

Resume execution of a disabled monitor:

<CodeGroup>

```bash cURL
curl -X POST "https://catchall.newscatcherapi.com/catchAll/monitors/{monitor_id}/enable" \
  -H "x-api-key: YOUR_API_KEY"
```

```python Python
from newscatcher_catchall import CatchAllApi

client = CatchAllApi(api_key="YOUR_API_KEY")
monitor_id = "3fec5b07-8786-46d7-9486-d43ff67eccd4"

# Enable monitor
response = client.monitors.enable_monitor(monitor_id)
print(response)
```

```typescript TypeScript
import { CatchAllApiClient } from "newscatcher-catchall-sdk";

const client = new CatchAllApiClient({ apiKey: "YOUR_API_KEY" });
const monitorId = "3fec5b07-8786-46d7-9486-d43ff67eccd4";

// Enable monitor
const response = await client.monitors.enableMonitor(monitorId);
console.log(response);
```

```java Java
import com.newscatcher.catchall.CatchAllApi;

CatchAllApi client = CatchAllApi.builder()
    .apiKey("YOUR_API_KEY")
    .build();

String monitorId = "3fec5b07-8786-46d7-9486-d43ff67eccd4";

// Enable monitor
var response = client.monitors().enableMonitor(monitorId);
System.out.println(response);
```

</CodeGroup>

Response:

```json
{
  "success": true,
  "message": "Monitor enabled successfully.",
  "monitor_id": "3fec5b07-8786-46d7-9486-d43ff67eccd4"
}
```

### Disable a monitor

Pause execution without deleting the monitor:

<CodeGroup>

```bash cURL
curl -X POST "https://catchall.newscatcherapi.com/catchAll/monitors/{monitor_id}/disable" \
  -H "x-api-key: YOUR_API_KEY"
```

```python Python
from newscatcher_catchall import CatchAllApi

client = CatchAllApi(api_key="YOUR_API_KEY")
monitor_id = "3fec5b07-8786-46d7-9486-d43ff67eccd4"

# Disable monitor
response = client.monitors.disable_monitor(monitor_id)
print(response)
```

```typescript TypeScript
import { CatchAllApiClient } from "newscatcher-catchall-sdk";

const client = new CatchAllApiClient({ apiKey: "YOUR_API_KEY" });
const monitorId = "3fec5b07-8786-46d7-9486-d43ff67eccd4";

// Disable monitor
const response = await client.monitors.disableMonitor(monitorId);
console.log(response);
```

```java Java
import com.newscatcher.catchall.CatchAllApi;

CatchAllApi client = CatchAllApi.builder()
    .apiKey("YOUR_API_KEY")
    .build();

String monitorId = "3fec5b07-8786-46d7-9486-d43ff67eccd4";

// Disable monitor
var response = client.monitors().disableMonitor(monitorId);
System.out.println(response);
```

</CodeGroup>

Response:

```json
{
  "success": true,
  "message": "Monitor disabled successfully.",
  "monitor_id": "3fec5b07-8786-46d7-9486-d43ff67eccd4"
}
```

<Note>
  To delete a monitor permanently, contact support at support@newscatcherapi.com
  with the `monitor_id`.
</Note>

## Webhooks

To get HTTP notifications for completed monitor jobs, add a `webhook` when 
creating the monitor (see [Create a monitor](#create-a-monitor)). You can update 
the webhook at any time using [update monitor webhook](#update-monitor-webhook) 
without recreating the monitor.

### Webhook payload

<Expandable title="Full payload structure">

When a monitor job completes, your endpoint receives:

```json
{
  "monitor_id": "3fec5b07-8786-46d7-9486-d43ff67eccd4",
  "reference_job_id": "295b95d8-6041-4f4b-b132-9f009fc6af70",
  "latest_job_id": "e2dd78b0-3189-48f3-9011-eef0ecd9aced",
  "records_count": 2,
  "jobs_processed": 3,
  "updated_at": "2025-11-07T11:25:33.877039",
  "cron_expression": "*/5 * * * *",
  "timezone": "UTC",
  "records": [
    {
      "record_id": "-39216465981207817",
      "record_title": "Homeplus Seeks Buyer, Receives LOI from AI Firm Harex Infotech",
      "added_on": "2025-11-07T11:25:33Z",
      "enrichment": {
        "schema_based_summary": "Harex Infotech acquired Homeplus Co. for null on null",
        "acquisition_date": null,
        "deal_type": "acquisition",
        "deal_status": "in talks",
        "confidence": "high",
        "acquired_company": "Homeplus Co.",
        "acquiring_company": "Harex Infotech"
      },
      "citations": [
        {
          "title": "Court extends Homeplus' rehabilitation plan submission deadline",
          "link": "https://n.news.naver.com/mnews/article/009/0005586304",
          "published_date": "2025-11-07T11:21:21Z",
          "job_id": "e2dd78b0-3189-48f3-9011-eef0ecd9aced",
          "added_on": "2025-11-07T11:25:33Z"
        }
      ]
    }
  ]
}
```

**Key fields:**

- `latest_job_id`: Use to retrieve full job results via
  `/catchAll/pull/{job_id}`
- `records_count`: Number of new records in this execution (after deduplication)
- `jobs_processed`: Total number of jobs executed by this monitor
- `records`: Array of new records from the latest job
- `records[].added_on`: Timestamp when this record was first collected
- `records[].citations[].added_on`: Timestamp when this citation was added

<Note>
  Webhooks fire on every job completion, even when `records_count` is 0 (no new
  events after deduplication).
</Note>

</Expandable>

### Authentication

<Expandable title="Authentication examples">

**Bearer token:**

```json
{
  "webhook": {
    "url": "https://api.example.com/catchall",
    "method": "POST",
    "headers": {
      "Authorization": "Bearer YOUR_TOKEN",
      "Content-Type": "application/json"
    }
  }
}
```

**Basic authentication:**

```json
{
  "webhook": {
    "url": "https://your-endpoint.com/webhook",
    "method": "POST",
    "auth": ["username", "password"]
  }
}
```

**Custom headers:**

```json
{
  "webhook": {
    "url": "https://your-endpoint.com/webhook",
    "method": "POST",
    "headers": {
      "X-Custom-Header": "value",
      "X-API-Key": "your-key",
      "Content-Type": "application/json"
    }
  }
}
```

</Expandable>

## See also

- [API reference](/v3/catch-all/endpoints/create-monitor)
- [Configure monitors](/v3/catch-all/how-to/configure-monitors)
- [Troubleshoot monitors](/v3/catch-all/how-to/troubleshoot-monitors)

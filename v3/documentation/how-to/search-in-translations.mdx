...
## Searching for all English and non-English content

To search for all English and non-English content in a single request, omit the
`lang` parameter and use both original and translated search fields:
...
- English articles where the original title or content contains the exact phrase
  "climate change".
- Non-English articles where the English translation of the title or content
  contains the exact phrase "climate change".

**Use case**: This is particularly useful when you want comprehensive global
coverage of a topic without being limited by language barriers. You can search
for English terms and find relevant content regardless of the original language
of publication.
...
3. **Remember the 2-option limit**: The `search_in` parameter accepts a maximum
   of 2 options.
4. **For global coverage**: To search all articles regardless of language, set
   `search_in` to "title_content,title_content_translated" without including the
   `lang` parameter.

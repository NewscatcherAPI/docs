...
| -------------- | ------------------------------------------------------------------------ |
| Theme          | General topic or category of the article                                 |
| Summary        | Concise overview of the article's content                                |
| Sentiment      | Separate scores for title and content sentiment                          |
| Named Entities | Identified persons, organizations, locations, and miscellaneous entities |
...
- `include_nlp_data` (boolean): Set to `true` to include the NLP layer for each
  article.
- `has_nlp` (boolean): Set to `true` to filter results to include only articles
  with an NLP layer.
...
Content tagging provides standardized categorization of news articles, enhancing
searchability and enabling more precise content filtering.

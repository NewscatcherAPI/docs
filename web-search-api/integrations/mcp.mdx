...
The MCP server resolves your API key from the first matching source in this
order:

1. `api_key` parameter passed directly in a tool call
2. `x-api-key` HTTP request header — **recommended** for all client
   configurations
3. `Authorization: Bearer <key>` HTTP request header
4. `?apiKey=YOUR_KEY` URL query parameter — used by Claude.ai because its
   connector UI does not support custom request headers
5. `CATCHALL_API_KEY` environment variable on the server host

Most client configurations use option 2 (the `x-api-key` header). Claude.ai
uses option 4 (the `apiKey` query parameter) automatically. The tools

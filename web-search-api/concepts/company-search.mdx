...
and scores each matching record per company. Use it to track specific companies â€” 
a portfolio, a competitor set, or any curated list â€” instead of running broad queries.
...
information â€” name, domain, alternative names, key persons â€” that the system
...
search mode; everything else about the job â€” query, validators, enrichments,
date ranges â€” works identically to a normal job.

## Create entities

Matching quality depends on what you provide. `domain` is the most reliable
signal â€” two companies can share a name, but not a domain.
...
Entities move through `pending â†’ enriching â†’ ready` (or `failed`) after
creation. You don't need to wait for individual entities to be ready before
creating a dataset â€” the dataset status is what matters before submitting a job.
...
processed independently â€” one failure does not affect the others.
...
organization â€” any invalid ID returns `400`.
...
Upload a CSV to create both entities and the dataset in a single request â€”
...
| Value | Behavior |
|---|---|
| `event_associated` | Only returns events where a tracked entity is the primary actor â€” the company that raised funding, was acquired, announced layoffs, etc. Passing references are excluded. |
...
Monitors created from an all-news job preserve this behavior â€” each recurring
...
for record in results.all_records:
    ...
    for entity in record.connected_entities:
        print(f"  {entity.name} â€” score {entity.ed_score}/10")
...
for (const record of results.all_records) {
  ...
  for (const entity of record.connected_entities ?? []) {
    console.log(`  ${entity.name} â€” score ${entity.ed_score}/10`);
...
results.getAllRecords().forEach(record -> {
    ...
    record.getConnectedEntities().ifPresent(entities ->
        entities.forEach(entity -> {
            System.out.printf("  %s â€” score %.0f/10%n",

...
<Note>
  ...
  search](/web-search-api/concepts/company-search) for setup
  instructions.
</Note>
...
### Scope a job to source groups

A source group is a curated, named allowlist of domains. Attaching one to a job
restricts fetching to those domains, which cuts noise without you having to
maintain a long domain list in every request.

Use source groups when you trust a specific set of publishers for a topic — for
example, tier 1 business media for M&A coverage, or a regional press set for
local events. Leave `source_groups` off to search the full index.

First, list the groups available to you with
[`GET /catchAll/source-groups`](/web-search-api/api-reference/jobs/list-source-groups).
The response includes public groups plus any organization-visibility groups your
organization has been granted access to. Each entry returns `slug`, `name`, and
`description`:
...
```json
{
  ...
  "source_groups": [
    {
      "slug": "tier-1-business-media",
      "name": "Tier 1 business media",
      "description": "Leading global business and financial news outlets."
    },
    {
      "slug": "apac-tech-press",
      "name": "APAC tech press",
      "description": "Technology publications covering the Asia-Pacific region."
    }
  ]
}
```
...
Then pass the `slug` values in `source_groups` when you submit the job. Listing
several groups searches the union of their domains. Maximum 20 groups per job.
...
curl -X POST "https://catchall.newscatcherapi.com/catchAll/submit" \
  ...
  -d '{
    "query": "Series B funding rounds for SaaS startups",
    "source_groups": ["tier-1-business-media"],
    "limit": 10
  }'
```
...
var job = client.jobs().createJob(
    SubmitRequestDto.builder()
        .query("Series B funding rounds for SaaS startups")
        .sourceGroups(List.of("tier-1-business-media"))
        .limit(10)
        .build()
);
```
...
The job results echo back the groups the job was scoped to in a `source_groups`
array, each with `slug`, `name`, and `description`, so you can tell what a job
searched without re-reading the submit request. Monitors expose the same array
on `reference_job`.
...
<Info>
  ...
  (1–10), and `relation`. See [Company search](/web-search-api/concepts/company-search) 
...
`citations` are serialized as a JSON column. For monitors, the equivalent
endpoint is

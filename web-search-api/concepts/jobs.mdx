...
![Job lifecycle diagram showing three flows: Basic (initialize optional â†’ submit â†’ get status â†’ pull results), Continue (continue â†’ get status â†’ pull results, extends a job submitted with limit), and Audit (list user jobs, returns all jobs for your API key).](/images/guides/job-lifecycle.png)
...
This endpoint is a preview â€” it shows you one example of what validators and
...
the job and starts processing. Only `query` is required â€” all other parameters
are optional.

Validators and enrichments are independent. You can define one without the
other â€” for example, provide custom enrichments and let the system generate
...
<Info>
  ...
  one entry per matched company â€” each with `entity_id`, `name`, `ed_score`
  (1â€“10), and `relation`. See [Company search](/web-search-api/concepts/company-search) 
...
It extends the limit without restarting the job â€” all analysis, validation, and
...
The difference tells you roughly how many candidates are still unprocessed â€” note 
...
print(f"Continued: {continued.previous_limit} â†’ {continued.new_limit} records")
...
console.log(
  `Continued: ${continued.previous_limit} â†’ ${continued.new_limit} records`,
);
...
System.out.println("Continued: " + continued.getPreviousLimit() + " â†’ " + continued.getNewLimit());
...
results. Deletion is idempotent â€” deleting an already-deleted job returns `200`.

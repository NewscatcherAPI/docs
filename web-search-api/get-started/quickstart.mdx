...
<Steps>
  ...
  <Step title="Create job">
    ...
    <Note>
      ...
      [`POST /catchAll/submit`](/web-search-api/api-reference/endpoints/jobs/create-job).
    </Note>

    <Tip>
      Use `limit=10` for faster testing, then continue processing more with 
      [`POST /catchAll/continue`](/web-search-api/api-reference/endpoints/jobs/continue-job) without 
      restarting.
    </Tip>
...
<Warning>
  ...
  acquisition queries. Use [`POST /catchAll/initialize`](/web-search-api/api-reference/endpoints/jobs/initialize-job) 
...
3. Explore the [API Reference](/web-search-api/api-reference/endpoints/jobs/create-job) for detailed

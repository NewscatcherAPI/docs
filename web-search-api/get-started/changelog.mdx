...
Adds two new Company Watchlist controls â€” association-type filtering and
all-news queries â€” along with performance improvements and API quality fixes.
...
- **`event_associated`** â€” only returns events where a tracked entity is the
  primary actor (e.g., the company that raised funding, was acquired, or
  announced layoffs). Passing references are excluded.
- **`mention`** â€” returns any event where a tracked entity appears, whether as
...
- `ed_association_type` on
  ...
  â€” Filter connected-entity results by association type. Requires
...
connected watchlist â€” no topic required. Wildcard queries (`*`) and generic
...
- `fetch_all_watchlist_news` on
  [`POST /catchAll/submit`](/web-search-api/api-reference/jobs/create-job)
  â€” When `true`, retrieves all news for connected watchlist entities without
  topic filtering. Requires `connected_dataset_ids`.
- `fetch_all_watchlist_news` on
  [`POST /catchAll/initialize`](/web-search-api/api-reference/jobs/initialize-job)
  â€” Preview validators and enrichments for an all-news watchlist query. Also
...
- [`POST /catchAll/validate`](/web-search-api/api-reference/jobs/validate-query)
  â€” Validate a query before submission.
...
- [`DELETE /catchAll/projects/{project_id}/resources/{resource_type}/{resource_id}`](/web-search-api/api-reference/projects/remove-resource)
  â€” Remove a resource from a project.
...
- **Delete jobs and monitors**: Remove jobs and monitors you no longer need.
  Deleted resources no longer appear in list results. Deletion is idempotent â€”
...
- [`GET /catchAll/monitors/{monitor_id}/status`](/web-search-api/api-reference/monitors/get-monitor-status-history)
  â€” Get monitor execution status history.
...
Introduces company search â€” track specific companies across CatchAll jobs by
...
- [`POST /catchAll/datasets`](/web-search-api/api-reference/datasets/create-dataset)
  â€” Create dataset from entity IDs.
- [`POST /catchAll/datasets/upload`](/web-search-api/api-reference/datasets/create-dataset-from-csv)
  â€” Create dataset from CSV.
...
- [`POST /catchAll/user/limits`](/web-search-api/api-reference/meta/get-plan-limits)
  â€” Get plan features and current usage.
...
- [`POST /catchAll/initialize`](/web-search-api/api-reference/jobs/initialize-job)
  â€” Get validator, enrichment, and date range suggestions.
- Enhanced
  [`POST /catchAll/submit`](/web-search-api/api-reference/jobs/create-job) â€” Now
  accepts date ranges, validators, and enrichments.
- Enhanced
  [`GET /catchAll/jobs/user`](/web-search-api/api-reference/jobs/list-user-jobs)
  â€” Now supports pagination.
...
- [`POST /catchAll/continue`](/web-search-api/api-reference/jobs/continue-job) â€”
...
- [`PATCH /catchAll/monitors/{monitor_id}`](/web-search-api/api-reference/monitors/update-monitor)
  â€” Update monitor webhook configuration.
...
Introduced
[monitors for automated queries](/web-search-api/concepts/monitors) â€”
...
- [`POST /catchAll/submit`](/web-search-api/api-reference/jobs/create-job) â€”
  Create jobs using a plain text queries.
- [`GET /catchAll/status/{job_id}`](/web-search-api/api-reference/jobs/get-job-status)
  â€” Check a job processing status.
- [`GET /catchAll/pull/{job_id}`](/web-search-api/api-reference/jobs/get-job-results)
  â€” Retrieve structured results with citations.

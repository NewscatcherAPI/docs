...
validator-driven queries judge evidence more literally. Existing jobs and monitors
...
- Job results echo the attached groups back as `source_groups`, and monitor results
...
See [Event tagging](/web-search-api/concepts/company-search#event-tagging).
...
accept the optional `project_id` query parameter, matching the job, monitor, and
dataset lists.
...
- Event timeframe validation gained a ±24-hour grace window, so an event reported
  just outside an exact UTC boundary is no longer dropped. Monitors keep their
...
- Watchlist fetching is bounded per job to keep large watchlists predictable, and
  monitor scheduling validates state at fire time so disabled monitors stop
  reliably.
</Update>

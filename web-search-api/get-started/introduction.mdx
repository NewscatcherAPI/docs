...
index, it delivers state-of-the-art recallâ€”finding all relevant events, not just 
top results.
...
<AccordionGroup>
  <Accordion title="Event-centric index" icon="database">
    CatchAll searches NewsCatcherâ€™s continuously updated web index of 2+ billion 
    web pages, optimized for finding real-world events (acquisitions, approvals, 
    incidents). The index is time-series by nature â€” each web page is recorded by 
  ...
  <Accordion title="Dynamic schemas" icon="flask">
    ...
    the `enrichment` object vary between jobsâ€”even with identical inputs.
  ...
  <Accordion title="Company search" icon="building">
    ...
    - Each record includes a `connected_entities` array with `entity_id`,
      `name`, `ed_score` (1â€“10), and `relation` for each matched company

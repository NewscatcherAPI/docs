...
â€” things that actually happen and get reported. The system queries a
...
- **Traditional web search:** "Information about clinical trials" â†’ Articles
  explaining what clinical trials are
- **CatchAll:** "Phase 3 clinical trial results for oncology drugs" â†’ Every
  specific trial result with efficacy data, statistical significance, and trial
  identifiers

<CodeGroup>
```text âœ… Good queries (describe events)
...
```text âŒ Poor queries (request articles or static data)
...
events that appear in published contentâ€”if something happens but isn't reported
...
```text âŒ Poor queries
Tech industry trends
(Analysis/commentary, not specific event)

AI developments
(Too vagueâ€”what kind of development?)
...
```text âŒ Too few details
Acquisitions
(No industry, geography, or thresholdâ€”millions of potential results)

Technology announcements
(What kind of announcement? Product launch? Partnership? Earnings?)
````

```text âŒ Too many details
Series C funding rounds for AI cybersecurity companies
in San Francisco over $100M announced on Tuesdays
(7 detailsâ€”very few results will match all constraints)
```
...
(Vague timeframe â€” use "last 7 days" or "this month" instead)

Corporate bankruptcies since 2020
(Multi-year range â€” likely exceeds plan's search depth)
```
</CodeGroup>
...
<CodeGroup>
```text âœ… Recommended: Single event type
...
```text âš ï¸ Works, but consider splitting
...
```text âŒ Difficult to work with

...
```
  ...
  <Step title="Assess event frequency vs schedule">
    ...
    - âœ… **Good match**: Daily monitor for `"AI startup funding rounds over $10M"` (multiple events per week)
    
    - âŒ **Frequency mismatch**: Daily monitor for `"Unicorn IPOs in biotech sector"` (rare events)
  ...
  <Step title="Adjust query or schedule">
    ...
    **Option 2 - Adjust query scope**: Expand to capture more events without losing business value.
    ...
    Create new reference job â†’ Create new monitor â†’ Disable old monitor.
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="Verify HTTPS">
    ...
    - âœ… `https://your-endpoint.com/webhook` 
    - âŒ `http://your-endpoint.com/webhook`
  </Step>
